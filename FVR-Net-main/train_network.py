...
def defineModel(model_type):
    ...
    if model_type == 'mynet2_50':
        model_ft = mynet.mynet2(layers=[3, 4, 6, 3])
    elif model_type == 'mynet2_101' or model_type == 'test':
        model_ft = mynet.mynet2(layers=[3, 4, 23, 3])
    elif model_type == 'mynet2_150' or 'mynet2_150_l1':
        model_ft = mynet.mynet2(layers=[3, 8, 36, 3])
        ...
        for phase in ['train', 'val']:
            ...
            for case_id, vol_tensor, frame_tensor, mat_tensor, dof_tensor, frameid_tensor in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    # np.savetxt('infoslabel_stats.txt', dof_label_stats)
...
if __name__ == '__main__':
    ...
    train_ids = np.genfromtxt(path.join(dataset_dir, 'train.txt'), dtype=str)
    val_ids = np.genfromtxt(path.join(dataset_dir, 'val.txt'), dtype=str)
    test_ids = np.genfromtxt(path.join(dataset_dir, 'test.txt'), dtype=str)
    data_dir = 'infos/sets/all_cases'
    data_transforms = {
    ...
    model_path = path.join(model_folder, '3d_best_Generator_{}.pth'.format(pretrain_model_str))  # 10
    # model_ft = mynet.mynet2
    model_ft = test_network.defineModel(model_type='mynet2_150', device=device)
    # normalize_dof = True
    normalize_dof = False
